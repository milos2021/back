<template>
  <div>
    ...
    >
      <p v-if="reportGenTime" class="reportTime">
        Report generated at: {{ reportGenTime }}
      </p>
      <v-data-table
        v-if="!chartMode"
        id="report1"
      ...
      >
        <template v-if="!isMobile" v-slot:header="{ props }">
          <tr>
            ...
            <th colspan="2" style="text-align: center">GGR Margin</th>
          </tr>
          <tr>
            <th
              class="mainHeader"
...
export default {
  ...
  data() {
    return {
      ...
      todayDateFile: "Betting_Report_" + new Date().toISOString() + ".xls",
      json_fields: {},
      json_data: [],
      json_meta: [
        [
  ...
  methods: {
    ...
    filterByMonth(criteria) {
      ...
      if(criteria!=0){
        this.aspectDefault=0;
        this.filters.aspect = 0;
      }
    },
    ...
    filterByAspect(criteria) {
      ...
      if (criteria != 2 && criteria != 4 && criteria!=5) {
        this.disableList.year = false;
        this.disableList.month = false;
      }
    },
    //Formatting json
    ...
    getData() {
      if (this.checkIfEligible()) {
        ...
        if (this.filters.aspect == 2) {
          monthFrom = this.formatDate(new Date());
          monthTo = this.formatDate(new Date());
        }
        ...
        axios
          .post(this.url + "/api/Tiket", {
            ...
            aspect: this.filters.aspect == 2 ? 3 : this.filters.aspect,
            realtime: this.filters.aspect == 2 
          })
          .then((response) => {
            if (!response.tiket) {
              this.chartData = null;
    ...
    fillChartData(criteria = 1) {
      ...
      Object.keys(this.tickets[0]).forEach((value, key) => {
        if (value != "Opseg" && value != "margin" && value != "CtrbBets") {
