...
def extractHandicap(outcomes):
    ...
    values = []
    for el in niz:
        if el.startswith("H1_Goal Spreads") and float(el.split("_")[2])%0.5==0 and not float(el.split("_")[2]).is_integer():
            available_handicaps.append(el.split("_")[2])
            values.append(float(el.split("_")[3]))
    if len(values)==0:
        return {"handicap":None, "prob":None}
    else:
        closestValue = min(values, key=lambda x:abs(x-0.5))
        for el in niz:
            if el.startswith("H1_Goal Spreads"):
                if(float(el.split("_")[3]) == closestValue):
                    handicap = float(el.split("_")[2])
