...
def find_best_matrix(lam1, lam2, x_prob, n=10):
    ...
    final = np.zeros((n, n))
    x_array, cov_array = [], []
    for cov in np.arange(0, 0.20, 0.01):
        for i in range(n):
            for j in range(n):
...
def real_time_calculation(gran, hend, gran_prob_under=None, hend_prob_under=None, x_prob=None, gran_first=None,
    ...
    hend = abs(hend)
    if hend == 0.5 and hend_prob_under<0.47:
    ...
    odds['15MG1'] = 0#final_15[1, 0] + final_15[0, 1]
    odds['15MG0'] = 0#final_15[0, 0]
    ...
    odds['PFG1'] = 0#lam1_first / ((lam1_first + lam2_first) / (1 - final_first[0, 0]))
    odds['PFG2'] = 0#lam2_first / ((lam1_first + lam2_first) / (1 - final_first[0, 0]))

    odds['1&FG1'] = 0#odds['K1'] * odds['FG1']
    odds['1&FG2'] = 0#odds['K1'] * odds['FG2']

    odds['2&FG1'] = 0#odds['K2'] * odds['FG1']
    odds['2&FG2'] = 0#odds['K2'] * odds['FG2']

    odds['X&FG1'] = 0#odds['KX'] * odds['FG1']
    odds['X&FG2'] = 0#odds['KX'] * odds['FG2']
...
if __name__ == '__main__':
    ...
    odds = real_time_calculation(2.5, 1.5, 0.279047688890869, 0.529107053713808, 0.101505)[-1]
    for od in odds:
