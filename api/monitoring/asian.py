...
obaranje_keys = {'98': 'param2', '96': 'param4', '94': 'param6', '92': 'param8', '90': 'param10'}
...
serbian_providers = {"admiral_rs","meridianbet","merkurxtip","mozzartbet","maxbet"}
...
def parseJson(js):
	...
	for match in js["data"]:
		...
		for prov in providers:
			if prov not in serbian_providers:
				dct[matchId][prov] = {}
		for provider in js['data'][match]['odds']:
			if provider not in serbian_providers:
...
def getDiffAsian(json_asian):
	...
	diffs = []
	with open(os.path.join(dir_path, 'asian_prepared.json')) as jsonfile:
...
def oddsJson(json_asian, json_prod, json_betradar):
		...
		for competition in json_prod[sport]:
			for match in json_prod[sport][competition]:
				...
				for k in match['odds']:
					...
					match['odds'][k]['obaranje'] = {'betradar' :{kk: 0 for kk in obaranje_keys.keys()}, 'asia' :{kk: 0 for kk in obaranje_keys.keys()}}
				if odds_betradar:
					for k in odds_betradar:
						...
						for group in groups:
							if k in group:
								ind = group.index(k)
								for p in match['odds'][k]['obaranje']['betradar']:
									if len(group) == 2:
										m, n = obaranje.obaranje_sabloni[obaranje_keys[p]]['osnovne 2types'][0], obaranje.obaranje_sabloni[obaranje_keys[p]]['osnovne 3types'][1]
										oborene = obaranje.obaranje_po_ishodima([odds_betradar[group[0]], odds_betradar[group[-1]]], m=m, n=n)
									elif len(group) == 3:
										m, n = obaranje.obaranje_sabloni[obaranje_keys[p]]['osnovne 3types'][0], obaranje.obaranje_sabloni[obaranje_keys[p]]['osnovne 3types'][1]
										oborene = obaranje.obaranje_po_ishodima([odds_betradar[group[0]], odds_betradar[group[1]],odds_betradar[group[-1]]], m=m, n=n)
									match['odds'][k]['obaranje']['betradar'][p] = oborene[ind]
				...
				if odds_asian:
					for k in odds_asian:
						match['odds'][k]['asian'] = round(1 / odds_asian[k], 2) if odds_asian[k] else 0
						match['odds'][k]['asian_difference'] = 0
						for group in groups:
							if k in group:
								ind = group.index(k)
								for p in match['odds'][k]['obaranje']['asia']:
									if len(group) == 2:
										m, n = obaranje.obaranje_sabloni[obaranje_keys[p]]['osnovne 2types'][0], obaranje.obaranje_sabloni[obaranje_keys[p]]['osnovne 3types'][1]
										oborene = obaranje.obaranje_po_ishodima([odds_asian[group[0]], odds_asian[group[-1]]], m=m, n=n)
									elif len(group) == 3:
										m, n = obaranje.obaranje_sabloni[obaranje_keys[p]]['osnovne 3types'][0], obaranje.obaranje_sabloni[obaranje_keys[p]]['osnovne 3types'][1]
										oborene = obaranje.obaranje_po_ishodima([odds_asian[group[0]], odds_asian[group[1]],odds_asian[group[-1]]], m=m, n=n)
									match['odds'][k]['obaranje']['asia'][p] = oborene[ind]
						if match['odds'][k]['production']:
							match['odds'][k]['asian_difference'] = 1 - match['odds'][k]['production']/match['odds'][k]['asian'] if match['odds'][k]['asian'] else 0
							if match['odds'][k]['asian_difference'] < 0:
								 match['level_asian'] += match['odds'][k]['asian_difference']
								 match['num_outcomes_asian'] += 1

	basic_json_prod = copy.deepcopy(json_prod)
	for sport in basic_json_prod:
		for competition in basic_json_prod[sport]:
			for match in basic_json_prod[sport][competition]:
				basic_odds = {}
				for x in match['odds'].keys():
					if x in ['KI 1', 'KI X', 'KI 2', 'UG 0-2', 'UG 3+']:
						basic_odds[x] = match['odds'][x]
				match['odds'] = basic_odds

	for i in list(range(1,16)):
		try:
			with open(os.path.join(dir_path, 'data.json'),'w') as jsonfile:
				json.dump(basic_json_prod, jsonfile)
				break
		except:
			time.sleep(3)
	else:
		with open(os.path.join(dir_path, 'error_log.txt'),'a') as err:
			err.write("[A] Main data write error - "+str(datetime.now())+"\n")
	# data = json_prod["FD"]
	# all = []
	...
	# 	try:
	# 		with open(os.path.join(dir_path, 'razrada_mini.json'), 'w') as outfile:
	...
	# else:
	# 	with open(os.path.join(dir_path, 'error_log.txt'),'a') as err:
	# 		err.write("[A] Razrada mini write error - "+str(datetime.now())+"\n")
	for i in list(range(1,16)):
		try:
			with open(os.path.join(dir_path, 'data_all.json'),'w') as jsonfile:
...
def writeProdChanges(json_prod_changes=None):
	...
	for i in list(range(1,100)):
		try:
			with open(os.path.join(dir_path, 'data_all.json')) as jsonfile:
		...
		with open(os.path.join(dir_path, 'error_log.txt'),'a') as err:
			err.write("[A] Main data read error - "+str(datetime.now())+"\n")
	match_ids_prod = []
		...
		for m in json_prod_changes['FD'][comp]:
			if str(m['matchId']) not in match_ids_prod:
				...
				for k in m['odds']:
					...
					m['odds'][k]['obaranje'] = {'betradar': {kk: 0 for kk in obaranje_keys.keys()}, 'asia': {kk: 0 for kk in obaranje_keys.keys()}}
	...
	# 	with open(os.path.join(dir_path, 'error_log.txt'),'a') as err:
	# 		err.write("[A] Razrada mini write error - "+str(datetime.now())+"\n")
	original = os.path.join(dir_path, 'data_all.json')
	target = os.path.join(dir_path, 'data_all_cp.json')
	shutil.copyfile(original, target)
	for i in list(range(1,11)):
		try:
			with open(os.path.join(dir_path, 'data_all.json'), 'w') as outfile:
...
def calculateOddsDiff(json_asian, json_prod_changes, json_betradar, mode):
	...
	if r['is_process_running'] == 0:
						...
						try:
							if str(match['betradar_id']) == str(m['matchId']):
								...
								for k in odds_betradar:
									...
									match['odds'][k]['betradar'] = round(1 / odds_betradar[k], 2) if odds_betradar[k] else 0
									match['odds'][k]['betradar_difference'] = 0
									match['odds'][k]['obaranje']['betradar'] = {kk: 0 for kk in obaranje_keys.keys()}
									for group in groups:
										if k in group:
											ind = group.index(k)
											for p in match['odds'][k]['obaranje']['betradar']:
												if len(group) == 2:
													q, n = obaranje.obaranje_sabloni[obaranje_keys[p]]['osnovne 2types'][0], obaranje.obaranje_sabloni[obaranje_keys[p]]['osnovne 3types'][1]
													oborene = obaranje.obaranje_po_ishodima([odds_betradar[group[0]], odds_betradar[group[-1]]], m=q, n=n)
												elif len(group) == 3:
													q, n = obaranje.obaranje_sabloni[obaranje_keys[p]]['osnovne 3types'][0], obaranje.obaranje_sabloni[obaranje_keys[p]]['osnovne 3types'][1]
													oborene = obaranje.obaranje_po_ishodima([odds_betradar[group[0]], odds_betradar[group[1]],odds_betradar[group[-1]]], m=q, n=n)
												match['odds'][k]['obaranje']['betradar'][p] = oborene[ind]
									if match['odds'][k]['production']:
										match['odds'][k]['betradar_difference'] = 1 - match['odds'][k]['production']/match['odds'][k]['betradar'] if match['odds'][k]['betradar'] else 0
										if match['odds'][k]['betradar_difference'] < 0:
							...
							with open(os.path.join(dir_path, 'error_log.txt'),'a') as err:
								err.write("[A] Greska u racunanju postojeceg - "+str(datetime.now())+"-"+str(m['matchId'])+"\n")
			else:
				for comp in json_prod['FD']:
					for match in json_prod['FD'][comp]:
						try:
							if str(match['betradar_id']) == str(m['matchId']):
								for odd in match['odds'].keys():
									if odd != 'code' and odd!='matchId' and odd!='startDate' and odd!='betradar_id':
										...
										match['odds'][odd]['obaranje']['betradar'] = {k: 0 for k in obaranje_keys.keys()}
										match['level_betradar'] = 0
										match['num_outcomes_betradar'] = 0
						except:
							with open(os.path.join(dir_path, 'error_log.txt'),'a') as err:
								err.write("[A] Greska u racunanju novog - "+str(datetime.now())+"-"+str(m['matchId'])+"\n")
		for m in diffs_cov:
			for comp in json_prod['FD']:
				for match in json_prod['FD'][comp]:
						try:
								...
								for prov in sport_default['FD']:
									if asian_prepared[str(match['matchId'])][prov]:
										...
										for k in odds_asian:
											match['odds'][k]['asian'] = round(1 / odds_asian[k], 2) if odds_asian[k] else 0
											match['odds'][k]['asian_difference'] = 0
											match['odds'][k]['obaranje']['asia'] = {kk: 0 for kk in obaranje_keys.keys()}
											for group in groups:
												if k in group:
													ind = group.index(k)
													for p in match['odds'][k]['obaranje']['asia']:
														if len(group) == 2:
															q, n = obaranje.obaranje_sabloni[obaranje_keys[p]]['osnovne 2types'][0], obaranje.obaranje_sabloni[obaranje_keys[p]]['osnovne 3types'][1]
															oborene = obaranje.obaranje_po_ishodima([odds_asian[group[0]], odds_asian[group[-1]]], m=q, n=n)
														elif len(group) == 3:
															q, n = obaranje.obaranje_sabloni[obaranje_keys[p]]['osnovne 3types'][0], obaranje.obaranje_sabloni[obaranje_keys[p]]['osnovne 3types'][1]
															oborene = obaranje.obaranje_po_ishodima([odds_asian[group[0]], odds_asian[group[1]], odds_asian[group[-1]]], m=q, n=n)
														match['odds'][k]['obaranje']['asia'][p] = oborene[ind]
											if match['odds'][k]['production']:
												match['odds'][k]['asian_difference'] = 1 - match['odds'][k]['production']/match['odds'][k]['asian'] if match['odds'][k]['asian'] else 0
												if match['odds'][k]['asian_difference'] < 0:
							...
							with open(os.path.join(dir_path, 'error_log.txt'),'a') as err:
								err.write(str(asian_prepared[str(match['matchId'])][prov]))
		...
		if mode is None:
			...
			basic_final = copy.deepcopy(final)
			for sport in basic_final:
				for competition in basic_final[sport]:
					for match in basic_final[sport][competition]:
						basic_odds = {}
						for x in match['odds'].keys():
							if x in ['KI 1', 'KI X', 'KI 2', 'UG 0-2', 'UG 3+']:
								basic_odds[x] = match['odds'][x]
			...
			for i in list(range(1,11)):
				...
				line1 = ""
				line2 = ""
				with open(os.path.join(dir_path, 'versions.txt'),'r') as ver:
					now = datetime.now()
					track = now.strftime("%H:%M:%S")
					finalLines = ""
					duzina = len(ver.readlines())
					if duzina == 0 or duzina==1:
						with open(os.path.join(dir_path, 'versions.txt'), 'a') as outfile:
							outfile.write(track+"\n" if duzina == 0 else track+"_last")
						with open(os.path.join(dir_path, 'razlika_'+str(track)+'.json'), 'w') as outfile:
							json.dump(basic_final, outfile)
				if duzina == 2:
					with open(os.path.join(dir_path, 'versions.txt'),'r') as ver:
						...
						while line:
							if 'last' not in line:
								line1 = now.strftime("%H:%M:%S")+"_last"
								os.rename(os.path.join(dir_path, 'razlika_'+line+'.json'), os.path.join(dir_path, 'razlika_'+line1[:-5]+'.json'))
								with open(os.path.join(dir_path, 'razlika_'+line1[:-5]+'.json'), 'w') as outfile:
									json.dump(basic_final, outfile)
							else:
								line2 = line[:-5].strip()
							line = ver.readline()
							cnt+=1
					with open(os.path.join(dir_path, 'versions.txt'), 'w') as outfile:
						outfile.write(line2+'\n'+line1)
				break
				# except:
				# 	time.sleep(3)
				...
				with open(os.path.join(dir_path, 'error_log.txt'),'a') as err:
					err.write("[A] Razlika write error - "+str(datetime.now())+"\n")
				with open(os.path.join(dir_path, 'process_running.json'), 'w') as outfile:
			...
			for i in list(range(1,16)):
				try:
					with open(os.path.join(dir_path, 'data.json'), 'w') as outfile:
						json.dump(basic_json_prod, outfile)
						break
				except:
					time.sleep(3)
			else:
			...
			for i in list(range(1,16)):
				...
				with open(os.path.join(dir_path, 'error_log.txt'),'a') as err:
					err.write("[A] All data write error - "+str(datetime.now())+"\n")
			with open(os.path.join(dir_path, 'process_running.json'), 'w') as outfile:
				json.dump({"is_process_running":0}, outfile)
			shutil.copyfile(original, target)
		else:
