...
if r['is_process_running']==0:
	...
	for row in podaci:
		...
		sql_comb_val = (row['Start'], row['Match ID'], row['Sport'], row['Home Team'], row['Away Team'], row['Outcome'], row['TotalImpactNumBets'], row['TotalImpactSumBet'], row['DirectImpactNumberBets'],row['DirectImpactSumBet'], row['DirectImpactWin'], str(datetime.now()+timedelta(hours=2)), row['Avg Odd'])
